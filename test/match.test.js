...
// 季桶加分（純加不減）：同名兩候選、僅放送季桶不同 → anime1 桶命中者勝出（消歧）。
test('季桶加分：同名同年不同季，命中 anime1 桶者勝出', () => {
  const parsed = parseTitle('某動畫 [03]');
  const subjects = [
    { id: 1, name: 'X', name_cn: '某動畫', date: '2024-01-10' }, // 2024冬
    { id: 2, name: 'X', name_cn: '某動畫', date: '2024-10-10' }, // 2024秋
  ];
  // 不傳桶：同分（無法消歧，FIFO 保第一個）
  const noBucket = rankCandidates(parsed, 2024, subjects);
  assert.equal(noBucket.ranked[0].score, noBucket.ranked[1].score);
  // 傳 anime1 桶 '2024秋' → id:2 勝出
  const withBucket = rankCandidates(parsed, 2024, subjects, ['2024秋']);
  assert.equal(withBucket.best.subject.id, 2);
  assert.ok(withBucket.best.score > withBucket.ranked[1].score);
});

// 守門：名稱完全不符的候選，即使季桶命中也不可因 bonus 翻過信心門檻。
test('季桶加分不翻盤：名稱不符者即使桶命中仍需確認', () => {
  const parsed = parseTitle('完全不同的動畫名 [01]');
  const subjects = [{ id: 9, name: 'Y', name_cn: '毫不相干的作品', date: '2024-10-10' }]; // 2024秋
  const r = rankCandidates(parsed, 2024, subjects, ['2024秋']);
  assert.equal(r.confident, false);
  assert.equal(r.needConfirm, true);
});
