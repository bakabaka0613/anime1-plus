...
test('英文序數季「-SECOND SEASON-」清除、baseName 不殘留破折號（鑽石王牌 act2）', () => {
  const r = parseTitle('鑽石王牌 act2 -SECOND SEASON-（第2季）');
  assert.equal(r.seasonNum, 2);
  assert.equal(r.baseName, '鑽石王牌 act2');
  assert.ok(!/season|second/i.test(r.baseName));
});
