...
為 [anime1.me](https://anime1.me/) 打造的瀏覽器增強油猴腳本：自動封面圖、觀看記錄與續播、海報卡片列表。
...
- 分類頁每集標示「已看 ✓ / 進度條」，頂部橫幅顯示**「上次看到第幾話」**並可一鍵繼續看（跨分頁也能跳，因看時已記下該集網址）。
- 分類頁多個內嵌播放器折疊成**上方選集列、下方單一播放器**，點集數即切換。
- **自動下一集**：看完自動切到下一集（折疊模式切選集列、單集頁倒數跳轉）。
...
### 其他
- **鍵盤快捷鍵**：`←/→` 快退/快轉（秒數可在選單調整、`Shift+←/→` 固定 10 秒）、`空白` 播放暫停、`N` 下一集、`F` 系統全螢幕、`W` 網頁全屏（填滿視窗）、`+/-` 調速。
