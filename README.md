...
- **播放速度記憶**。
- **追番清單浮動面板**（右下 📺）：列出在追的動畫（名稱用繁體）、一鍵繼續看；**有更新的（已追平後又出新集）置頂並標「+N 新集」**——只對「曾追到當時最新集」的動畫提醒，落後沒追完的不會跳。已看完最後一集者，依首頁集數欄狀態顯示**「已到最新進度」（仍標連載中）或「已看完」（已完結）**。
- **追番清單刪除模式**：**按住 Shift 點 📺** 開啟面板，每部右側出現 🗑 鈕，確認後**單獨刪除該動畫的觀看進度（保留封面快取）**；不按 Shift 開啟則無刪除鈕，避免誤刪。
- **網頁全屏按鈕**：播放器右上角明顯按鈕（亦可按 `W`），填滿視窗的網頁全屏；播放中閒置時會跟控制列一起自動淡出，滑鼠移入才顯示。
...
- **只同步進度**：不含封面快取（各端各自向 Bangumi 重抓）與裝置設定（快進秒數、卡片寬度等保持各端獨立）。
